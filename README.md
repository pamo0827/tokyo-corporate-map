...
本アプリの企業情報は、公的制度に由来する以下のデータを組み合わせて表示しています。

### gBizINFO（経済産業省）

gBizINFO は、経済産業省が運営する **法人活動情報の公式データベース** です。国税庁の法人番号制度を土台に、国の各機関が保有する法人の基本情報や活動情報を名寄せして無料公開しています。上場・非上場を問わず幅広い法人をカバーしており、本アプリの収録企業の土台になっています。

- 運営: 経済産業省（公式サイト: https://info.gbiz.go.jp/ ）
- 名寄せキー: 法人番号（13桁）
- 取得できる主な情報: 法人名・所在地・代表者・設立日・資本金・従業員数・事業概要のほか、補助金/助成金の受給、調達（入札）実績、表彰、届出・認定、特許などの公開情報
- 利用方法: 無料の API トークンを取得し、サーバー側でプロキシして利用（トークンを秘匿）

### EDINET DB（edinetdb.jp ／ EDINET 由来の財務データ）

EDINET（Electronic Disclosure for Investors' NETwork）は、金融庁が運営する **有価証券報告書など法定開示書類の電子提出・公開システム** です。上場企業等が提出した有価証券報告書・決算短信の内容が公開されています。本アプリでは、その EDINET 開示データを構造化して API 提供しているサードパーティ・サービス **edinetdb.jp** を通じて、各社の詳細な財務・指標を取得しています。

- 大元: 金融庁 EDINET（https://disclosure.edinet-fsa.go.jp/ ）／ API 提供元: edinetdb.jp（https://edinetdb.jp/ ）
- 名寄せキー: EDINET コード（例: `E04426`）
- 対象: 上場・EDINET 登録のある企業（本アプリでは約 3,800 社）
- 取得できる主な情報:
  - 損益: 売上高・各段階利益（営業/経常/当期純）・BS/PL/CF
  - 指標: 信用格付・信用スコア・ROE・自己資本比率・PER/EPS/BPS
  - 配当: 1株配当・配当性向・配当総額
  - 株式: 発行済株式数・自己株式・政策保有株式
  - 人材/ガバナンス: 従業員数・平均年収・平均年齢・女性管理職比率・男性育休取得率・男女賃金差・役員報酬 など
- 制約: 無料プランは 100 リクエスト/日。取得結果は DB にキャッシュし、同一企業の再表示では消費しません。

### geolonia/japanese-addresses（住所→緯度経度）

町丁目レベルの住所マスタを無料公開しているオープンデータ。緯度経度を持たない企業について、選択時に住所からジオコーディングして地図に表示します（全国対応・番地までは特定しない近似）。
...
本番稼働中: **https://web-production-35dcf.up.railway.app** （Railway 上で Flask + MySQL を稼働）

DB 接続情報・API トークンは Railway の環境変数（`DB_HOST` / `DB_USER` / `DB_PASSWORD` / `DB_NAME` / `GBIZ_API_TOKEN` / `EDINET_API_KEY`）で設定しており、`DB_*` は MySQL サービスへの参照変数で解決しています。
